 ...
 *
 * ADMIN SPLIT 2026-09-01 ─ the counterpart is apps/dashboard/lib/auth-cookie.js, which
 ...
 * app carried apps/dashboard's pattern, a request here with no session of its own
 * would still find apps/dashboard's cookie, conclude a session had outlived its
 ...
 *
 * WHY THE NAME DOES NOT START WITH `sb-`. apps/dashboard/proxy.js matches
 * /^sb-.+-auth-token(\.\d+)?$/ across the whole jar too. An `sb-` prefixed name
 * would match it in development, so apps/dashboard's optimistic gate would see a
