 ...
 * from apps/dashboard, a stray second helper — whichever call runs first wins for the
 * lifetime of the page, and it would lock in the DEFAULT storage key. From then
 * on this app reads and writes apps/dashboard's cookie, silently, and the split is
 ...
 * reuse apps/dashboard's session in development — its own /login would be unreachable,
 ...
 *
 * WHY NOT `sb-admin-auth-token`. apps/dashboard/proxy.js matches
 * /^sb-.+-auth-token(\.\d+)?$/ across the whole jar. An sb- prefixed name would
 * match it in development, so apps/dashboard's optimistic gate would see a cookie it
 ...
 * clean.
 *
 * The counterpart is apps/dashboard/lib/supabase.js, which takes no options and
